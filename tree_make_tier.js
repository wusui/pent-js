 ...
 */
const getNewLeafInfo = tree => [tree.map(x => x.offspring.points),
 ...
 */
const newNextTierNodes = info => info.map(x => ({point: x[0], parent: x[1]}));
...
const getParentLinkValues = nextLevel => pentUtils.zip(
  (x => x[0].map(x => x.length))(nextLevel), nextLevel[1]);
 ...
 */
const getFlatParentLinks = linkInfo => linkInfo.map(x =>
  Array(x[0]).fill(x[1]));
